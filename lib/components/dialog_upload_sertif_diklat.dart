...
class DialogUploadSertifDiklat extends StatefulWidget {
  final List<dynamic> refJenisKursus;
  final Function(bool status) isUploading;
  const DialogUploadSertifDiklat(
      {super.key, required this.refJenisKursus, required this.isUploading});

  @override
  State<DialogUploadSertifDiklat> createState() => _DialogUploadSertifDiklatState();
}

class _DialogUploadSertifDiklatState extends State<DialogUploadSertifDiklat> {
  File? fileResult;
  ...
  Future<void> _addUsulan({required Map<String, dynamic> data}) async {
    try {
      ...
      final response = await Rekues().postData(
        url: "profile/diklat_struktural/usulan/create",
